...
import { type Assignment, DEFAULT_ASSIGNMENTS } from '@/types';
...
const AssignmentsPage = () => {
  const [assignments, setAssignments] = useStore<EnhancedAssignment[]>('assignments', DEFAULT_ASSIGNMENTS);

  useEffect(() => {
    trackFeatureUsage('Assignments & Deadlines', 'Utility');
  }, []);
          ...
          {groupedPending.length > 0 ? (
            ...
            <div className="cs-card p-14 text-center border-dashed border-border/40 bg-secondary/10 flex flex-col items-center justify-center rounded-[2.5rem] shadow-sm">
              <div className="w-20 h-20 rounded-full bg-slate-500/5 flex items-center justify-center mb-6">
                <FileText className="w-10 h-10 text-slate-500/40" />
              </div>
              <p className="text-base font-black text-foreground mb-1 tracking-tight">No assignments created.</p>
              <p className="text-sm text-muted-foreground font-medium opacity-60">Add your first assignment above to get started.</p>
            </div>
          )}
        </div>
      )}
