...
const CGPACalculator = () => {
  ...
  // Canonical branch mapper for syllabus linking:
  // CSE & CSE-ICP -> 'CSE' (Same Syllabus)
  // AIML & AIML-ICP -> 'AIML' (Same Syllabus)
  const getCanonicalBranch = (branchStr: string): 'CSE' | 'AIML' => {
    const b = (branchStr || '').toLowerCase().trim();
    if (b.includes('aiml') || b.includes('ai')) {
      return 'AIML';
    }
    return 'CSE';
  };

  const getPrefilledSubjectsForSemester = (semNum: number, rawBranch: string): Subject[] => {
    const branch = getCanonicalBranch(rawBranch);
  ...
  const handleSelectSemester = async (num: number) => {
    ...
    const canonicalBranch = getCanonicalBranch(profile.className || 'CSE');
    ...
    try {
      ...
      if (templateSubjects.length === 0) {
        templateSubjects = getPrefilledSubjectsForSemester(num, canonicalBranch);
      }
      ...
      if (existing) {
        // If existing record contains dummy placeholders or fewer subjects than the template, upgrade it!
        const isDummyRecord = existing.subjects.some(s => s.name.startsWith('Theory Core') || s.name.startsWith('Core Lab'));
        
        let mergedSubjects: Subject[] = [];
        if (isDummyRecord || existing.subjects.length < templateSubjects.length) {
          mergedSubjects = templateSubjects.map(tempSub => {
          ...
          mergedSubjects = existing.subjects;
        }
  ...
  return (
    <div className="cs-page pb-8">
      ...
      {/* Tab Switcher: SGPA vs CGPA */}
      <div className="cs-card p-1 flex items-center mb-6 bg-secondary/50 border-border">
          ...
          <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-300">
            <div className="grid grid-cols-2 gap-3">
              {[
                ...
                const canonicalBranch = getCanonicalBranch(profile.className || 'CSE');
                const defaultCount = getPrefilledSubjectsForSemester(sem.num, canonicalBranch).length;
                const activeCount = record && record.subjects.length > 3 ? record.subjects.length : defaultCount;
                
                const hasRecord = !!record;
                const isCompleted = hasRecord && record.sgpa > 0;
                const isInProgress = hasRecord && record.sgpa === 0;

                const statusLabel = isCompleted ? 'Completed' : isInProgress ? 'In Progress' : 'Prefilled';
                const detailLabel = isCompleted 
                  ? `SGPA: ${record.sgpa.toFixed(2)}` 
                  : `${activeCount} Subjects`;
