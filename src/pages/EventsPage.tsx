...
import { MapPin, CalendarDays, ExternalLink, Link as LinkIcon, ChevronLeft, Building2, Search, SlidersHorizontal, Clock, Download, FileText } from 'lucide-react';
import type { Event } from '@/types';
import BackButton from '@/components/BackButton';
import { store, useStore } from '@/lib/store';
import { trackFeatureUsage, trackEventInteraction } from '@/lib/telemetry';
...
export default function EventsPage() {
  ...
  const [events] = useStore<Event[]>('events', []);

  useEffect(() => {
    store.pullFromSupabase().catch(console.error);
    trackFeatureUsage('Events & Registrations', 'Campus Life');
  }, []);

  useEffect(() => {
    if (location.state && location.state.highlightId && events.length > 0) {
    ...
    <div className="cs-page min-h-screen pb-24">
      ...
      <div className="space-y-4">
        {filteredEvents.length === 0 ? (
          <div className="text-center p-12 bg-card border border-border/40 rounded-[2rem]">
            <CalendarDays className="w-12 h-12 text-muted-foreground/30 mx-auto mb-3" />
            <h3 className="text-sm font-bold text-foreground">No events available</h3>
            <p className="text-xs text-muted-foreground mt-1">Check back later for new campus events and gatherings.</p>
          </div>
        ) : (
           ...
           <div className="min-h-screen p-4 sm:p-6 flex flex-col items-center">
             ...
             <div className="w-full max-w-2xl bg-card rounded-[2.5rem] border border-border/60 shadow-2xl overflow-hidden mb-12 animate-in slide-in-from-bottom-8 duration-500">
                ...
                <div className="p-8 pt-8">
                   ...
                   {selectedEvent.attachmentsList && selectedEvent.attachmentsList.length > 0 && (
                     <div className="mt-8 border-t border-border/30 pt-6">
                       <h4 className="text-[11px] font-bold uppercase tracking-widest text-muted-foreground mb-4">Event Documents & Files ({selectedEvent.attachmentsList.length})</h4>
                       <div className="space-y-3">
                         {selectedEvent.attachmentsList.map((att, idx) => (
                           <div key={idx} className="flex items-center justify-between p-4 rounded-2xl bg-secondary/30 border border-border/30 shadow-sm">
                             <div className="flex items-center gap-3 truncate">
                               <FileText className="w-5 h-5 text-primary shrink-0" />
                               <span className="text-sm font-bold text-foreground truncate">{att.name || `Attachment ${idx + 1}`}</span>
                             </div>
                             <button
                               onClick={() => {
                                 trackEventInteraction(selectedEvent.id, 'download');
                                 downloadFile(att.url, att.name || `Event_Document_${idx + 1}`);
                               }}
                               className="flex items-center gap-1.5 px-4 py-2.5 rounded-xl bg-primary text-primary-foreground text-xs font-bold shadow-sm shadow-primary/20 hover:opacity-90 active:scale-95 transition-all"
                             >
