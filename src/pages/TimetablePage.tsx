...
const TimetablePage = () => {
  ...
  useEffect(() => {
    trackFeatureUsage('Timetable & Schedule', 'Academic');
  }, []);
  ...
  useEffect(() => {
    ...
    if (docId) {
      ...
      // Default to student's department and year from profile
      try {
        const savedProfile = localStorage.getItem('campusync_profile');
        if (savedProfile) {
          const profile = JSON.parse(savedProfile);
          if (profile.department) {
            const mappedDept = profile.department.includes('CSE') ? 'CSE / CSE ICP' : 'AIML / AIML ICP';
            setSelectedDept(mappedDept);
          }
          if (profile.year) {
            setSelectedYear(Number(profile.year));
            setSelectedSemester(`Semester ${profile.year}-1`);
          }
        }
      } catch (err) {
        console.error('Failed to load profile for default timetable:', err);
      }
    }
  }, [searchParams]);
