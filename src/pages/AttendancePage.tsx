...
import { type AttendanceRecord, DEFAULT_ATTENDANCE_RECORDS } from '@/types';
...
const AttendancePage = () => {
  ...
  const [records, setRecords] = useStore<AttendanceRecord[]>('attendance', DEFAULT_ATTENDANCE_RECORDS);
  ...
  useEffect(() => {
    trackFeatureUsage('Attendance Tracker', 'Academic');
        ...
        <div className="cs-card p-10 text-center mb-8 border-dashed border-border flex flex-col items-center justify-center rounded-[2rem] shadow-sm">
          {searchQuery ? (
            ...
            <>
              ...
              <p className="text-base font-bold text-foreground mb-1">Start by adding your first subject.</p>
              <p className="text-sm text-muted-foreground">Add subjects to start tracking your overall attendance.</p>
            </>
          )}
        </div>
      )}
