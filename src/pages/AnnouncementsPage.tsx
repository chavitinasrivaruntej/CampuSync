...
import { trackFeatureUsage, trackAnnouncementInteraction } from '@/lib/telemetry';
...
export default function AnnouncementsPage() {
  ...
  const [announcements, setAnnouncements] = useStore<Announcement[]>('announcements', []);

  useEffect(() => {
    store.pullFromSupabase().catch(console.error);
    trackFeatureUsage('Announcements & Feeds', 'Content');
  }, []);

  useEffect(() => {
    if (location.state && location.state.highlightId && announcements.length > 0) {
      const id = location.state.highlightId;
      setTimeout(() => {
        const el = document.getElementById(`announcement-${id}`);
        if (el) {
          el.scrollIntoView({ behavior: 'smooth', block: 'center' });
          el.classList.add('ring-4', 'ring-primary/50', 'animate-pulse', 'scale-[1.02]');
          setTimeout(() => {
            el.classList.remove('ring-4', 'ring-primary/50', 'animate-pulse', 'scale-[1.02]');
          }, 3000);
          
          const target = announcements.find(a => a.id === id);
          if (target) {
            setSelectedAnnouncement(target);
            // Optionally clear the state so it doesn't re-trigger on remount
            window.history.replaceState({}, document.title);
          }
        }
      }, 100);
    }
  }, [location.state, announcements.length]);
  ...
  const markAsRead = (a: Announcement) => {
    ...
    trackAnnouncementInteraction(a.id, 'view');
  };
  ...
  const filteredAnnouncements = useMemo(() => {
    return announcements.filter(a => {
      ...
      const course = a.course || 'All';
      const year = a.year || 'All';
      const priority = (a.priority || 'general').toLowerCase();
      const category = a.category || 'General';

      if (filters.course !== 'All' && course !== 'All' && course !== filters.course) return false;
      if (filters.year !== 'All' && year !== 'All' && year !== filters.year) return false;
      if (filters.priority !== 'All' && priority !== filters.priority.toLowerCase()) return false;
      if (filters.category !== 'All' && category !== filters.category) return false;
      if (filters.attachmentOnly && (!a.attachmentURL && (!a.attachmentsList || a.attachmentsList.length === 0))) return false;
  ...
  const { pinned, recent, archived } = useMemo(() => {
    ...
    sorted.forEach(a => {
      ...
      if (a.isPinned) {
        ...
        if (!isNaN(exp.getTime()) && today.getTime() > exp.getTime()) {
          archivedFlow.push(a);
        } else {
          recentFlow.push(a);
        }
             ...
             <div className="w-full max-w-2xl bg-card rounded-[2.5rem] border border-border/60 shadow-2xl overflow-hidden mb-12 animate-in slide-in-from-bottom-8 duration-500">
              ...
              <div className="p-6 border-b border-border/30 bg-secondary/5 relative overflow-hidden">
                ...
                <div className="p-6 pt-5">
                   ...
                   {selectedAnnouncement.attachmentURL && (
                     <div className="mb-8 p-4 rounded-2xl bg-secondary/20 border border-border/30 flex flex-col gap-4 animate-in slide-in-from-top-2 duration-300">
                        ...
                        <div className="grid grid-cols-2 gap-3">
                          ...
                          <button 
                            onClick={() => {
                              trackAnnouncementInteraction(selectedAnnouncement.id, 'download');
                              downloadFile(selectedAnnouncement.attachmentURL!, `Official_Notice_${selectedAnnouncement.title.replace(/[^a-zA-Z0-9]/g, '_')}.${selectedAnnouncement.attachmentType || 'pdf'}`);
                            }}
                   ...
                   {selectedAnnouncement.attachmentsList && selectedAnnouncement.attachmentsList.length > 0 && (
                     <div className="mb-8 p-4 rounded-2xl bg-secondary/20 border border-border/30 space-y-3">
                       <h4 className="text-[10px] font-bold uppercase tracking-widest text-muted-foreground">Attached Documents & Media ({selectedAnnouncement.attachmentsList.length})</h4>
                       <div className="space-y-2">
                         {selectedAnnouncement.attachmentsList.map((att, idx) => (
                           <div key={idx} className="flex items-center justify-between p-3 rounded-xl bg-card border border-border/40 text-xs">
                             <div className="flex items-center gap-3 truncate">
                               <FileText className="w-4 h-4 text-primary shrink-0" />
                               <span className="font-bold text-foreground truncate">{att.name || `Attachment ${idx + 1}`}</span>
                             </div>
                             <button
                               onClick={() => downloadFile(att.url, att.name || `Attachment_${idx + 1}`)}
                               className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-primary text-primary-foreground text-[10px] font-bold hover:opacity-90 active:scale-95 transition-all"
                             >
